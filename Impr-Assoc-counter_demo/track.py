...
def parse_count_lines_file(count_lines_file):
    ...
    lines = [[eval(coord) for coord in line] for line in lines]
    return lines
...
if __name__ == "__main__":
    ...
    if args.yolo_version == 'yolov8':
        ...
        # load YOLOv8
        logger("loading YOLOv8 model from: {}", MODEL)
        ...
        # load YOLO-NAS
        logger("loading YOLO-NAS model from: {}", MODEL)
    ...
    video_info = sv.VideoInfo(SOURCE_VIDEO_PATH)
    ...
    trace_annotator = sv.TraceAnnotator(thickness=2, color=(0, 255, 0), trace_length=300)
    ...
    def callback(frame: np.ndarray, frame_id: int, color_calib_device='cpu') -> np.ndarray:
        ...
        if args.yolo_version == 'yolov8':
            ...
            detections = detections[np.isin(detections.class_ids, selected_classes)]
        elif args.yolo_version == 'yolo-nas':
        ...
        if detections.tracker_ids.size is None:
            annotated_frame = frame.copy()
        else:
            annotated_frame = trace_annotator(scene=frame.copy(), detections=detections)
        ...
        ''' Log Time'''
        if frame_id % 20 == 0:
            logger.info('Processing frame {}/{} ({:.2f} fps)'.format(frame_id, video_info.length, max(1e-5, fps_monitor.average_fps)))
